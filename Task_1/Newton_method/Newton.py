...
def diff_eduation(
        x):  # Задаём производную нашего уравнения (Можно было её расчитать с помощью библеотеки "sympu" но я посчитал на листочке и задал сам, что бы для проверки данного задания не приходилось скачивать сторонние библеотеки)
